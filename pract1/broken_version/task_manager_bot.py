...
def format_tasks(user_id: str, data: Dict[str, List[dict]]) -> str:
    ...
    lines = []
    for i, task in enumerate(tasks, start=0): #при выводи списка задач нумерация должна идти с 1, но будет начинаться с 0, что сломает взаимодействие по номеру задачи
        status = "✅" if task["done"] else "❌"
        lines.append(f"{i}. {task['title']} {status}")
    return "\n".join(lines)

@dp.message(Command("star")) # допущена ошибка в названии стартовой команды /start
...
async def process_add(message: types.Message, state: FSMContext):
    ...
    save_data(data)
    await state.clear()
    await message.answer(f"Задача удалена ✅\n{message.text}", reply_markup=main_kb) # Пишет что задача удалена вместо задача добавлена
...
async def delete_task(message: types.Message, state: FSMContext):
    ...
    if text == "У вас пока нет задач ✅":
        await message.answer(text)
        #return - при отсутсвии return продублируется сообщение о том что задач нету и всё равно предложит ввести номер удаляемой задачи
...
async def process_done(message: types.Message, state: FSMContext):
    ...
    #save_data(data) - без этой строки не сохраняется выполненный статус задачи
    await state.clear()
